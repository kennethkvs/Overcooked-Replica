class Level2 extends Level{
  ...
  Level2(){ // Argumentless constructor
    thisLevel = 2;
    gameMaxTimeSec = 4 * 60 + 30;
  
    Player1 = new Player(160, 760, false);
    Player2 = new Player(340, 760, true);
    
    // Assign ingredients    
    interactiveObjects.add(new Storage(145, 865, 0.05, "Tomato"));
    interactiveObjects.add(new Storage(245, 865, 0.05, "Mushroom"));
    interactiveObjects.add(new Storage(345, 865, 0.05, "Potato"));
    ...
    interactiveObjects.add(new ServingTray(950, 450, new String[]{"mushroomSoup", "tomatoSoup", "potatoSoup"}, 15000, this));
  ...
  void constrainMotion(Player player){
    ...
    //println(circleRectCollision(p.posX, p.posY, 50, 200, 200, 620, 500));
    
    if(circleRectCollision(player.X, player.prevPosY, 50, 200 + player.speed, 200 + player.speed, 620 - player.speed - 1, 500 - player.speed - 1))
      player.X = player.prevPosX;
      
    if(circleRectCollision(player.X, player.Y,  50, 200 + player.speed, 200 + player.speed, 620 - player.speed - 1, 500 - player.speed - 1))
      player.Y = player.prevPosY;
  }
  
  // Checks If player has moved into the middle table/wall.
  boolean circleRectCollision(float objX, float objY, float radius, float rx, float ry, float rw, float rh) {
    float testX = objX;
    float testY = objY;
  
    // which edge is closest?
    if (objX < rx)         testX = rx;      // test left edge
    else if (objX > rx + rw) testX = rx+rw;   // right edge
    if (objY < ry)         testY = ry;      // top edge
    else if (objY > ry + rh) testY = ry+rh;   // bottom edge
  
    // get distance from closest edges
    float distX = objX - testX;
    float distY = objY - testY;
